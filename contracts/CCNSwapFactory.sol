...
contract CCNSwapERC20 is ICCNSwapERC20 {
    using SafeMath for uint;

    string public constant name = 'CCNSwap';
    string public constant symbol = 'CCNSwap LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'CCNSwap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'CCNSwap: INVALID_SIGNATURE');
        _approve(owner, spender, value);
    }
}

contract CCNSwapPair is ICCNSwapPair, CCNSwapERC20 {
    using SafeMath  for uint;
    ...
    modifier lock() {
        require(unlocked == 1, 'CCNSwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'CCNSwap: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'CCNSwap: FORBIDDEN');
        // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(- 1) && balance1 <= uint112(- 1), 'CCNSwap: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = ICCNSwapFactory(factory).feeTo();
        feeOn = feeTo != address(0) && ICCNSwapFactory(factory).getPairRate(address(this)) != 9;
        uint _kLast = kLast;
        // gas savings
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint denominator = rootK.mul(ICCNSwapFactory(factory).getPairRate(address(this))).add(rootKLast);
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'CCNSwap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        // using balances ensures pro-rata distribution
        require(amount0 > 0 && amount1 > 0, 'CCNSwap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'CCNSwap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves();
        // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'CCNSwap: INSUFFICIENT_LIQUIDITY');
        ...
        {// scope for _token{0,1}, avoids stack too deep errors
            address _token0 = token0;
            address _token1 = token1;
            require(to != _token0 && to != _token1, 'CCNSwap: INVALID_TO');
        ...
        require(amount0In > 0 || amount1In > 0, 'CCNSwap: INSUFFICIENT_INPUT_AMOUNT');
        {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint balance0Adjusted = balance0.mul(1e4).sub(amount0In.mul(ICCNSwapFactory(factory).getPairFees(address(this))));
            uint balance1Adjusted = balance1.mul(1e4).sub(amount1In.mul(ICCNSwapFactory(factory).getPairFees(address(this))));
            require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1e8), 'CCNSwap: K');
        }
...
contract CCNSwapFactory is ICCNSwapFactory {
    ...
    constructor(address _feeToSetter) public {
        ...
        initCodeHash = keccak256(abi.encodePacked(type(CCNSwapPair).creationCode));
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'CCNSwapFactory: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'CCNSwapFactory: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'CCNSwapFactory: PAIR_EXISTS');
        // single check is sufficient
        bytes memory bytecode = type(CCNSwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ICCNSwapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'CCNSwapFactory: FORBIDDEN');
        feeTo = _feeTo;
    }

    function setFeeToSetter(address _feeToSetter) external {
        require(msg.sender == feeToSetter, 'CCNSwapFactory: FORBIDDEN');
        require(_feeToSetter != address(0), "CCNSwapFactory: FeeToSetter is zero address");
        feeToSetter = _feeToSetter;
    }

    function addPair(address pair) external returns (bool){
        require(msg.sender == feeToSetter, 'CCNSwapFactory: FORBIDDEN');
        require(pair != address(0), 'CCNSwapFactory: pair is the zero address');
    ...
    function setFeeRateNumerator(uint256 _feeRateNumerator) external {
        require(msg.sender == feeToSetter, 'CCNSwapFactory: FORBIDDEN');
        require(_feeRateNumerator <= 30, "CCNSwapFactory: EXCEEDS_FEE_RATE_DENOMINATOR");
    ...
    function setPairFees(address pair, uint256 fee) external {
        require(msg.sender == feeToSetter, 'CCNSwapFactory: FORBIDDEN');
        require(fee <= 30, 'CCNSwapFactory: EXCEEDS_FEE_RATE_DENOMINATOR');
        pairFees[pair] = fee;
    }
    ...
    function setDefaultFeeToRate(uint256 rate) external {
        require(msg.sender == feeToSetter, 'CCNSwapFactory: FORBIDDEN');
        require(rate > 0 && rate <= 10, "CCNSwapFactory: FEE_TO_RATE_OVERFLOW");
    ...
    function getReserves(address tokenA, address tokenB) public view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = ICCNSwapPair(pairFor(tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) public pure returns (uint amountB) {
        require(amountA > 0, 'CCNSwapFactory: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'CCNSwapFactory: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, address token0, address token1) public view returns (uint amountOut) {
        require(amountIn > 0, 'CCNSwapFactory: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'CCNSwapFactory: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, address token0, address token1) public view returns (uint amountIn) {
        require(amountOut > 0, 'CCNSwapFactory: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'CCNSwapFactory: INSUFFICIENT_LIQUIDITY');
        uint256 fee = getPairFees(pairFor(token0, token1));
    ...
    function getAmountsOut(uint amountIn, address[] memory path) public view returns (uint[] memory amounts) {
        require(path.length >= 2, 'CCNSwapFactory: INVALID_PATH');
